...
- **Proof**: We will utilize an exchange argument. Let the cost $C(S)$ be the sum of waiting times (which is equivalent to minimizing the average). The waiting time for the $j$-th job in a schedule is the sum of the processing times of all jobs before it.The total cost can be expressed as:$$C(S) = \sum_{i=1}^n (n-i) \cdot p_i$$(Where $p_i$ is the processing time of the job in the $i$-th position. The first job is waited on by $n-1$ others, the second by $n-2$ others, etc. Note: The prompt defines waiting time as sum of prior jobs, so the $i$-th job waits for the $i-1$ jobs before it. This formula represents the contribution of each job $p_i$ to the waiting times of others).Argument:Assume the optimal schedule $S$ is not sorted by shortest-job-first.This means there must be two adjacent jobs in the schedule, say at positions $k$ and $k+1$, such that $p_k > p_{k+1}$ (a longer job comes immediately before a shorter one).Exchange these two jobs to create a new schedule $S'$.Analyze the Change:Jobs before position $k$ and after $k+1$ are not affected.The job originally at $k+1$ now waits less (it moves forward).The job originally at $k$ now waits more (it moves back), but it is a "longer" job being moved behind a "shorter" one.Mathematically, the change in total cost is determined by the term $(n-k)p_k + (n-(k+1))p_{k+1}$.In $S'$ (swapped), the contribution is $(n-k)p_{k+1} + (n-(k+1))p_k$.Change $\Delta = \text{New} - \text{Old} = p_{k+1} - p_k$.Since we assumed $p_k > p_{k+1}$, then $\Delta < 0$.This means the cost decreased. This contradicts the assumption that the unsorted schedule $S$ was optimal.Therefore, the optimal schedule must be sorted by processing time (Shortest-Job-First).
